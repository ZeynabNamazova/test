...
select name,surname,salary
from MyEmployers
where salary=(select min(salary)from MyEmployers);     --minimum emek haqqi kime verildiyi gosterilir

select name,surname,salary
from MyEmployers
where salary=(select max(salary)from MyEmployers);      --maksimum emek haqqinin kime verildiyi gosterilir
